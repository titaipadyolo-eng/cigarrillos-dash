...
def resolver_url_real(url):
    """Los links de Google News RSS son redirecciones encriptadas, no la URL
    del artículo. Esta función las decodifica a la URL real del medio."""
    if "news.google.com" not in url:
        return url
    try:
        resultado = new_decoderv1(url, interval=1)
        if resultado and resultado.get("status") and resultado.get("decoded_url"):
...
def procesar():
    ...
    nuevos, duplicados, descartados, sin_texto, errores = 0, 0, 0, 0, 0
    ...
    for entrada in entradas:
        ...
        texto = descargar_texto(url_real)
    ...
    print(
        ...
        f"| descartados (no relevantes)={descartados} | sin texto descargable={sin_texto} "
        f"| errores={errores}"
    )
